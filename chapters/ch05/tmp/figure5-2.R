...
# read shapefile
wmap <- readOGR(dsn="tmp/map/ne_110m_land", layer="ne_110m_land")
...
grat <- readOGR("tmp/map/ne_110m_graticules_all", layer="ne_110m_graticules_15") 
grat_df <- fortify(grat)
bbox <- readOGR("tmp/map/ne_110m_graticules_all", layer="ne_110m_wgs84_bounding_box") 
bbox_df<- fortify(bbox)
grat_robin <- spTransform(grat, CRS("+proj=robin"))  # reproject graticule
grat_df_robin <- fortify(grat_robin)
bbox_robin <- spTransform(bbox, CRS("+proj=robin"))  # reproject bounding box
bbox_robin_df <- fortify(bbox_robin)
countries <- readOGR("tmp/map/ne_110m_admin_0_countries", layer="ne_110m_admin_0_countries") 
...
setEPS()
postscript(file="figures/793725c05f002-horiz.eps", paper="special", 
           width=12, height=5, horizontal=FALSE) 
grid.arrange(equirect, poly, wink, ncol=3, clip=T)
dev.off()

setEPS()
postscript(file="figures/793725c05f002-vert.eps", paper="special", 
           width=5, height=12, horizontal=FALSE) 
