...
dest.ips = read.csv("dest.ips")
colnames(dest.ips) = c("IP")

# take a look at the reliability of the IP address entries
ggplot(data=av.df[av.df$IP %in% dest.ips$IP,]) +
  geom_histogram(aes(Reliability),binwidth=1)

# can also do it with a simple table()
...
g.asn = graph.asn(ips,av.df,show.labels=TRUE)


dest.pct = data.frame(
  table(factor(av.df[av.df$IP %in% ips,]$Country))/
    length(ips))
colnames(dest.pct)=c("fw","pct")
av.gt6.pct = data.frame(
  table(factor(av.df[av.df$Reliability > 6,]$Country))/
    nrow(av.df[av.df$Reliability > 6,]))
colnames(av.gt6.pct)=c("av","pct")
compare.df = merge(dest.pct,av.gt6.pct,by.x="fw",by.y="av")
colnames(compare.df)=c("CC","fw","av")

#install.packages("reshape")
library(reshape)
compare.m = melt(compare.df)
colnames(compare.m) = c("CC","Source","Pct")

ggplot(data=compare.m) + 
  geom_bar(aes(CC,Pct,fill=Source),stat="identity",position="dodge") +
  labs(title="Percent Of Malicious Nodes By Data Source And Country")


source.ips = read.csv("/suda/chapters/ch04/data/source.ips")
colnames(source.ips) = c("IP")
ips = as.character(av.df[(av.df$IP %in% source.ips$IP),]$IP)
g.cc.src = graph.cc(ips,av.df)

src.pct = data.frame(
  table(factor(av.df[av.df$IP %in% ips,]$Country))/
    length(ips))
colnames(src.pct)=c("fw","pct")
av.pct = data.frame(table(factor(av.df$Country))/nrow(av.df))
colnames(av.pct)=c("av","pct")
compare.df = merge(src.pct,av.pct,by.x="fw",by.y="av")
colnames(compare.df)=c("CC","fw","av")
...
ggplot(data=compare.m) + 
  geom_bar(aes(CC,Pct,fill=Source),stat="identity",position="dodge") +
  labs(title="% Malicious Nodes By Data Source & Country (inbound)")

compare.df$difference = (compare.df$fw / compare.df$av)
ggplot(data=compare.df) + 
  geom_bar(aes(x=CC, y=difference), stat="identity") +
  labs(title="CCs % Diff Between Inbound FW & AlienVault Full DB")